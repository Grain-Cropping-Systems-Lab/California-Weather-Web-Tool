...
render_map <- function(shapefile, markers){
	renderGoogle_map({
		google_map(key = Sys.getenv("MAPS_API_KEY"),
			...
			add_markers(data = data.frame(lat = markers$lat, lon = markers$lon),
									draggable = TRUE,
									update_map_view = FALSE)
	})
...
update_map <- function(markers, map_id){
...
map_mod_ui <- function(id){
	ns <- NS(id)
	google_mapOutput(ns("map")) %>% shinycssloaders::withSpinner(type = 6, color="#005fae")
}
	...
	moduleServer(
		...
		function(input, output, session){
			...
			observeEvent(input$map_geolocation,{
				...
				if(is.null(input$map_geolocation)){
